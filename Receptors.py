...
class tactile_receptors():
    ...
    def tactile_units_simulating_v0(self,stimulus):  # the simulation code of previous version
        ...
        '''
                ...
                #----IF spiking model----
                dV = 20*self.Vnf[:,ptime]- gl*(self.Va[:,ptime]-El)/Cm
                self.Va[:,ptime] = self.Va[:,ptime -1] + dV*self.dt
                # in case we exceed threshold
                self.Va[(self.Va[:,ptime]>thresh),ptime-1]=0.04
                self.Va[(self.Va[:,ptime]>thresh),ptime]=El
                
        elif (spk=='if'):
            self.spike_trains=[]
            '----Acquiring spikes from the signal of Va-----'
    ...
    def tactile_units_simulating(self,stimulus):  # the simulation code of current version
        ...
        for ptime in range(1,self.t.size-3*wd-steps):  #时间 t.size
            ...
            #----Spiking synthesizer----'
            self.Vs[:,ptime]=self.Vnf[:,int(ptime/(self.Ta/self.dt))*int((self.Ta/self.dt))] #smapling the singal of Vnf
            fs=self.Vs[:,ptime]*self.Kf; Ts=1/(fs+1)
            self.Vf[:,ptime]=(1-fs*(ptime*self.dt%Ts))*(self.Vs[:,ptime]>0)
            self.Vr[:,ptime]=1*(self.Vf[:,ptime]>0.5)+0*(self.Vf[:,ptime]<=0.5) 
