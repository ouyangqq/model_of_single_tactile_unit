...
def consuming_time_acquire(sampling_rate,srate_c,repeat_c):
    #wave_double_set(x1,x2)
    #single_simulate()
    T=200   #ms
...
def Print_fig2(frows,fcol,sampling_rate):
    #wave_double_set(x1,x2)
    #single_simulate()
    T=200   #ms
    dt=1/float(sampling_rate) #ms
...
for srate_c in range(ratesets.size):
    for repeat_c in range(repeats):
        consuming_time_acquire(ratesets[srate_c],srate_c,repeat_c)
