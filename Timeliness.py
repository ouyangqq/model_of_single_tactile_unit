...
def plot_f():
    order=[0.5,1,2,5,10]
    Average=[0,0,0,0,0]
    TSA1=[2300,1000,600,420,300]
    TRA1=[1810,1100,500,360,260]
    TPC=[1300,680,410,290,180]
    for i in range(5):
        Average[i]=np.average([TSA1[i],TRA1[i],TPC[i]])
    plt.errorbar(order,TSA1,yerr=[240,102,80,40,21],label=u'SA1',fmt='g.--',linewidth=1,capsize=3)
    plt.errorbar(order,TRA1,yerr=[200,102,80,40,21],label=u'RA1',fmt='b.--',linewidth=1,capsize=3)
    plt.errorbar(order,TPC,yerr=[180,102,80,40,21],label=u'PC',fmt='r.--',linewidth=1,capsize=3)
    plt.errorbar(order,Average,yerr=[180,102,80,40,21],label=u'Average',fmt='k.-',linewidth=1.5,capsize=3)
    plt.yticks([0,500,1000,1500,2000,2500]) 
    plt.xticks([0.5,2,5,10])  
    plt.xlabel(u"Simpling rate (kHz)", fontsize=10)
    plt.ylabel(u"MNT (Num)", fontsize=10)
...
def plot_f2(clolor_buf,marker_buf,type_buf):
    for j in range(len(ttype)):  
        ...
        #Tc_buf2[j,:]=np.mean(num)/np.mean(Tc_buf1[j,:,:],axis=1)
        Ter_buf2[j,:]=np.std(np.mean(num)/Tc_buf1[j,:,:],axis=1)
        #plt.errorbar(srate,Tc_buf2[j,:],yerr=Ter_buf2[j,:],label=type_buf[j],color=clolor_buf[j],linewidth=1,fmt=marker_buf[j+1],markersize=6,capsize=2)
        plt.plot(srate,Tc_buf2[j,:],label=type_buf[j],color=clolor_buf[j],linewidth=1,marker=marker_buf[j+1],markersize=6)
        #plt.yticks([0,500,1000,1500,2000,2500],fontsize=8) 
...
np.save('timeliness_buf.npy',Tc_buf) 
'''
Tc_buf=np.load("timeliness_buf.npy")
#
